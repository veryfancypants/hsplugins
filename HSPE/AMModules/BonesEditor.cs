...
namespace HSPE.AMModules
{
    public class BonesEditor : AdvancedModeModule
    {
        ...
        public class TransformData
        {
            ...
            public static TransformData interpolate(TransformData a, TransformData b, float factor)
            {
                TransformData c = new TransformData(a);
                if (a.position.hasValue || b.position.hasValue)
                    c.position.value = Vector3.LerpUnclamped(a.position.hasValue ? a.position.value : a.originalPosition.value,
                        b.position.hasValue ? b.position.value : b.originalPosition.value,
                        factor);
                if (a.scale.hasValue || b.scale.hasValue)
                    c.scale.value = Vector3.LerpUnclamped(a.scale.hasValue ? a.scale.value : a.originalScale.value,
                        b.scale.hasValue ? b.scale.value : b.originalScale.value,
                        factor);
                if (a.rotation.hasValue || b.rotation.hasValue)
                    c.rotation.value = Quaternion.SlerpUnclamped(a.rotation.hasValue ? a.rotation.value : a.originalRotation.value,
                        b.rotation.hasValue ? b.rotation.value : b.originalRotation.value,
                        factor);
                return c;
            }
        }
        ...
        public Dictionary<GameObject, TransformData> _dirtyBones = new Dictionary<GameObject, TransformData>();
        ...
        public void SetBoneTransform(Transform bone, TransformData tdnew)
        {
            TransformData td = this.SetBoneDirty(bone.gameObject);
            td.scale = tdnew.scale;
            td.rotation = tdnew.rotation;
            td.position = tdnew.position;
            if (!td.originalPosition.hasValue)
                td.originalPosition = bone.localPosition;
            if (!td.originalRotation.hasValue)
